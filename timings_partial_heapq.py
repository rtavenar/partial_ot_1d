...
from partial_nb import partial_ot_1d as partial_ot_1d_nb, partial_ot_1d_elbow as partial_ot_1d_elbow_nb
...
for n in values_for_n:
    ...
    for _ in range(n_repeat):
        ...
        # Ours using numba + elbow
        t0 = time.time()
        partial_ot_1d_elbow_nb(x, y)
        sum_time_ours_elbow += (time.time() - t0)
    ...
    timings_ours_elbow.append(sum_time_ours_elbow / n_repeat)
...
plt.loglog(values_for_n, timings_ours_elbow, label="Our method using heap queues and numba (elbow)")
...
plt.savefig("timings_partial_nb_hq.pdf")
