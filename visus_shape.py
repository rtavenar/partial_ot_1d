    ...
    for idx_pc, percent in enumerate([5, 7]):
        for idx_n, n_source in enumerate([9 * 1000, 10 * 1000]):
            ...
            for idx_m, method in enumerate(baselines.keys()):
                if not os.path.exists(f"results_icp/{problem}_p{percent}_n_source{n_source}_baseline-{method}.npz"):
                    continue

                data_results = np.load(f"results_icp/{problem}_p{percent}_n_source{n_source}_baseline-{method}.npz")
            ...
            plt.tight_layout()
            plt.savefig(f"shape_figs/viz_{problem}_p{percent}_n_source{n_source}.pdf")
